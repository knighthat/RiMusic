...
import me.knighthat.logger.Log
...
fun OtherSettings() {
    ...
    Column(
        ...
        val exportLauncher = rememberLauncherForActivityResult(
            ...
            context.applicationContext.contentResolver.openOutputStream( uri ).use { outStream ->
                if( outStream == null ) return@rememberLauncherForActivityResult

                Logger.dump( Log.Level.DEBUG ).forEach {

                    val format = "${it.formattedDate} / ${it.level.name} - ${it.message}"
                    outStream.write( format.toByteArray() )
                    outStream.flush()
                }
            }
        }
        ...
        ButtonBarSettingEntry(
            ...
            onClick = {
                ...
                exportLauncher.launch( "${dateFormat.format( Date() )}.log" )
            }
        )
